...
public class MinecraftSignHandler extends BaseThingHandler {
    ...
    private void hookupListeners(MinecraftServerHandler bridgeHandler) {
        signSubscription = bridgeHandler.getSignsRx().flatMap(new Func1<List<SignData>, Observable<SignData>>() {
            @Override
            public Observable<SignData> call(List<SignData> signs) {
                return Observable.from(signs);
            }
        }).filter(new Func1<SignData, Boolean>() {
            @Override
            public Boolean call(SignData sign) {
                return config.getName().equals(sign.getName());
            }
        }).subscribe(new Action1<SignData>() {

            @Override
            public void call(SignData sign) {
